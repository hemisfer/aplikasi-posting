                    ...
                    <form class="forms-sample" action="{{ route('update-posting') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id" class="form-control" id="">
                              ...
                              @foreach ($user as $usr)
                                  <option value="{{ $usr->id }}" @if($data->user_id == $usr->id) {{'selected'}} @endif >{{ $usr->name}}</option>
                              @endforeach
                            </select>
                          </div>
                        <div class="form-group">
                            <label for="exampleInputName1">Judul</label>
                            <input type="text" class="form-control  @error('judul') is-invalid @enderror" id="exampleInputName1" name="judul" value="{{ $data->judul }}" placeholder="Judul">
                            @error('judul')
                        ...
                        @enderror
                          ...
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control  @error('topik') is-invalid @enderror" id="exampleInputEmail3" name="topik" value="{{ $data->topik }}" placeholder="Topik">
                            @error('topik')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        @enderror
                          ...
                          <div class="form-group">
                            ...
                            <textarea type="text" class="form-control  @error('isi') is-invalid @enderror" id="editor" name="isi" placeholder="Isi">{{ $data->isi}}</textarea>
                            @error('isi')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
